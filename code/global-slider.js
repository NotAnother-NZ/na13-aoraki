(function () {
  ...
  Slider.prototype.applyListStyles = function () {
    ...
    injectOnce(
      "rt-ss-" + listUID,
      `[data-rt-ss-id="${listUID}"]::-webkit-scrollbar{display:none}[data-rt-ss-id="${listUID}"].is-dragging{cursor:grabbing !important;user-select:none}[data-rt-ss-id="${listUID}"] img,[data-rt-ss-id="${listUID}"] a,[data-rt-ss-id="${listUID}"] ${this.conf.item}{user-select:none;-webkit-user-drag:none}`
    );
  };
